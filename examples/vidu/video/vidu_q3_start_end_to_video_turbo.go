...
func runViduQ3StartEndToVideoTurbo(ctx context.Context, svc xai.Service, model xai.Model) error {
	op, err := svc.Operation(model, xai.GenVideo)
	if err != nil {
		return err
	}
	op.Params().
		Set(vidu.ParamPrompt, "Dragon lands on a rock").
	...
	results, err := xai.Call(ctx, svc, op, newViduOptions(svc, "demo-user-q3-start-end-turbo"), progressPrinter("q3-start-end-turbo"))
	if err != nil {
		return err
	}
	printVideoResults(string(model), "start-end-to-video-turbo", results)
	return nil
}
