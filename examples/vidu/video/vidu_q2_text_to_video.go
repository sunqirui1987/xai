...
func runViduQ2TextToVideo(ctx context.Context, svc *vidu.Service, model xai.Model) error {
	op, err := svc.Operation(model, xai.GenVideo)
	if err != nil {
		return err
	}
	op.Params().
		Set(vidu.ParamPrompt, "A drone shot over snow mountains, sunrise, volumetric light, epic wide shot.").
		Set(vidu.ParamSeed, 11).
