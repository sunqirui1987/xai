// Seedance (Qiniu/Qnagic overseas BytePlus Dreamina) video generation example.
//
// Qnagic docs:
...
//
// This example uses byteplus/dreamina-seedance-2-0-260128. The Qiniu provider
// does not run asset review for this model, so reference URLs are submitted
// directly to the video task API.
package main
...
func main() {
	...
	if apiKey == "" {
		fmt.Println("Set QINIU_API_KEY to call Qiniu overseas Seedance (see spec/seedance/provider/qiniu/README.md).")
		os.Exit(1)
	}

	qiniu.Register(apiKey)
	...
	model := xai.Model(seedance.ModelByteplusDreaminaSeedance20)
