...
type ParamBuilder interface {
	...
	// 1) User-defined tools defined with `ToolDef` and refer by their unique names.
	// 2) Std-tools: refer by StdTool objects (e.g. WebSearchTool, etc.)
	//
	// If you include `tools` in your API request, the model may return `tool_use`
	// content blocks that represent the model's use of those tools. You can then run
	// those tools using the tool input generated by the model and then optionally
	// return results back to the model using `tool_result` content blocks.
	Tools(tools ...any) ParamBuilder
...
type Provider interface {
	...
	// ToolIsDefined checks if a tool with the given name has been defined.
	ToolIsDefined(name string) bool
