...
// Client is the HTTP client for Qiniu Audio API (ASR, TTS).
type Client struct {
...
// WithRetry enables retry with exponential backoff.
...
// NewClient creates a new Qiniu Audio API client.
...
func (c *Client) logDebug(format string, args ...any) {
	if c.debugLog && c.logger != nil {
		c.logger.Printf("[qiniu-audio] "+format, args...)
	}
}
...
func (c *Client) doRequest(ctx context.Context, method, endpoint string, body any) ([]byte, error) {
	...
	if body != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("qiniu-audio: failed to marshal request body: %w", err)
		}
	}

	curlCmd := c.buildCurlCommand(method, url, reqBodyBytes)
	if c.logger != nil {
		c.logger.Printf("[qiniu-audio] curl command:\n%s", curlCmd)
	} else {
		fmt.Println(curlCmd)
	}
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		if err != nil {
			return nil, fmt.Errorf("qiniu-audio: failed to create request: %w", err)
		}
		...
		if err != nil {
			lastErr = fmt.Errorf("qiniu-audio: request failed: %w", err)
		...
		resp.Body.Close()
		if err != nil {
			lastErr = fmt.Errorf("qiniu-audio: failed to read response body: %w", err)
			continue
		}
...
func (e *APIError) Error() string {
	...
	result := fmt.Sprintf("qiniu-audio: [%s] %s", code, msg)
	if e.RequestID != "" {
