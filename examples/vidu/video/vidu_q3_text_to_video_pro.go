...
func runViduQ3TextToVideoPro(ctx context.Context, svc xai.Service, model xai.Model) error {
	...
	results, err := xai.Call(ctx, svc, op, newViduOptions(svc, "demo-user-q3-text-pro"), progressPrinter("q3-text-pro"))
	if err != nil {
		return err
	}
	printVideoResults(string(model), "text-to-video-pro", results)
	return nil
}
